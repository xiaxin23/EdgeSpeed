...
def weighted_allocate_res(target_list, weight, fraction_num):
    duplicated_list = []
...
def weighted_allocate(target_list, fraction_num):
    num_per_fraction = len(target_list) // fraction_num
    res = []
    for i in range(fraction_num-1):
        list_to_append = list(target_list[i*num_per_fraction: (i+1)*num_per_fraction])
        res.append(list_to_append)
    list_to_append = list(target_list[(fraction_num-1)*num_per_fraction:])
    res.append(list_to_append)
    return res
